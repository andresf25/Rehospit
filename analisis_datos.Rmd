...
data_rehosp %>%
  mutate(pago_hosp = ifelse(quirur == "Si", outlier(mydata = ., 
         ...
         dias_uce = outlier(mydata = ., 
                            value = "dias_uce", 
                            q_min = 0, q_max = 0.99)) -> data_rehosp
...
Transformacion de variables de acuerdo al WOE

```{r}

data_rehosp %>%
  mutate(pago_hosp = case_when( pago_hosp <= 107410 ~ "[0,107410]",
         ...
         dias_hosp = case_when( dias_hosp = 1 ~ "[1]",
                                dias_hosp = 2 ~ "[2]",
                                dias_hosp = 3 ~ "[3]",
                                dias_hosp = 4 ~ "[4]",
                                dias_hosp >= 5 & dias_hosp <= 7 ~ "[5,7]",
                                dias_hosp >= 8 & dias_hosp <= 30 ~ "[8,30]")) -> data_rehosp
...
<h3 style="text-align: center;"><a id="Modelo">Modelo</a></h3>
...
Vemos que la representacion para la categoría positiva es alrededor del 2% de la información. En ete caso vamos a realizar un tratamiento de la información que permita aumentar la clase minoritaria, sin utilizar soluciones genéricas como reducir la clase mayoritaria al nivel de la clase minoritaria.

Pära este caso vamos a utilizar la técnica SMOTE (Synthetic Minority Oversampling Method), la cual genera
nuevas instancias artificiales de la clase minoritaria interpolando los valores de las instancias minoritarias más cercanas a una dada.
...
Estimando el modelo
```{r}
mylogit <- glm(rehosp_oms ~ pago_hosp + quirur + dias_uce + dias_uci + dias_hosp + estrato, data = training, family = "binomial")
...
```{r}
pred <- predict(mylogit, newdata = testing, type = "response")
...
ROCRpred = prediction(predictTrain, training$rehosp_oms)
...
table(training$rehosp_oms, predictTrain > 0.5)
```
