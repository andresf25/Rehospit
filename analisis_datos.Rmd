...
```{r}
ggplot(data = data_rehosp, aes(x = edad)) +
  geom_histogram(aes(y = ..density.., fill = ..count..)) +
  scale_fill_gradient(low = "#DCDCDC", high = "#7C7C7C") +
  stat_function(fun = dnorm, colour = "firebrick",
                args = list(mean = mean(data_rehosp$edad),
                            sd = sd(data_rehosp$edad))) +
  ggtitle("Histograma + curva normal teórica") +
  theme_bw()


filter(rehospmuestra, dias_uci > 0) -> rehospmuestra
filter(data_rehosp, dias_uci > 0) -> drehosp_prueb


#Muestra sin reemplazo
rehospmuestra<- data_rehosp %>%
  sample_n(size=5000,replace=FALSE)

ks.test(data_rehosp$edad, "pnorm", mean=mean(data_rehosp$edad), sd=sd(data_rehosp$edad))


library("nortest")
library("ggplot2")

lillie.test(x = data_rehosp$edad)

library("tseries")
jarque.bera.test(x = data_rehosp$edad)
```
...
newData <- SMOTE(rehosp_oms ~ ., as.data.frame(data_rehosp2), perc.over = 300, perc.under = 200)
