...
<<<<<<< HEAD
list.of.packages <- c("readxl", "dplyr", "ggplot2", "ggcorrplot", "VIM", "RColorBrewer", "Information", "knitr", "kableExtra", "gridExtra", "skimr", "nortest", "GGally", "plotly", "lattice", "DMwR", "caTools")
=======
list.of.packages <- c("readxl", "dplyr", "ggplot2", "ggcorrplot", "VIM", "RColorBrewer", "Information", "knitr", "kableExtra", "gridExtra", "skimr", "nortest", "GGally", "plotly", "DMwR")
>>>>>>> 7ea3b765665f46e07dc4c56d001a976b41b2c7d6
...
```{r}
data_rehosp %>%
  mutate(pago_hosp = ifelse(quirur == "Si", outlier(mydata = ., 
                                                    value = "pago_hosp", 
                                                    q_min = 0.01, q_max = 0.99), pago_hosp),
         pago_hosp = ifelse(quirur == "No", outlier(mydata = ., 
                                                    value = "pago_hosp", 
                                                    q_min = 0.01, q_max = 0.99), pago_hosp),
...
filter(data_rehosp, quirur == "No") -> z

z$pago_hosp = outlier(z, "pago_hosp", 0.01, 0.99)

quantile(z$pago_hosp, 
         c(0.01,0.99), 
         na.rm = TRUE)
...
```
Seleccionar variables para modelo
```{r, message = FALSE, warning = FALSE, echo = FALSE, eval =TRUE}
...
MultiPlot(IV, IV$Summary$Variable[10:14])
...
```{r}
set.seed(88)
split <- sample.split(data_rehosp$rehosp_oms, SplitRatio = 0.75)

# Create training and testing sets
data_rehospTrain = subset(data_rehosp, split == TRUE)
data_rehospTest = subset(data_rehosp, split == FALSE)

#evidenciando clase desbalanceada
prop.table(table(data_rehospTrain$rehosp_oms))
table(data_rehospTrain$rehosp_oms)

data_rehospTrain$rehosp_oms <- as.factor(data_rehospTrain$rehosp_oms)

## Smote : Synthetic Minority Oversampling Technique To Handle Class Imbalancy In Binary Classification
## now using SMOTE to create a more "balanced problem"
data_rehospTrain <- SMOTE(rehosp_oms ~ ., as.data.frame(data_rehospTrain), perc.over = 300, perc.under = 200)

#verificar clase balanceada
prop.table(table(data_rehospTrain$rehosp_oms))
table(data_rehospTrain$rehosp_oms)

data_rehospTrain <- na.omit(data_rehospTrain)
 
mylogit <- glm(rehosp_oms ~ pago_hosp + quirur + dias_uce + dias_uci + dias_hosp + estrato, data = data_rehospTrain, family = "binomial")
 
predictTrain = predict(mylogit, type="response")
 
table(data_rehospTrain$rehosp_oms, predictTrain > 0.5)
 
pred <- predict(mylogit, newdata = data_rehospTest, type = "response")
 
y_pred_num <- ifelse(pred > 0.5, 1, 0)
y_pred <- factor(y_pred_num, levels=c(0, 1))
y_act <- data_rehospTest$rehosp_oms
 
mean(y_pred == y_act)  # 66%
 
library(ROCR)
 
 
ROCRpred = prediction(predictTrain, data_rehospTrain$rehosp_oms)
