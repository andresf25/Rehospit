...
```{r}
table(as.matrix(testing[, 7]), y_pred > 0.4)
```
...
cv_train <- cv.glmnet(x_train, y_train, family = "binomial", nfolds = 10, type.measure = "class")
plot(cv.lasso)
```

La gráfica muestra el error de validación cruzada de acuerdo con el registro de lambda. La línea vertical punteada a la izquierda indica que el registro del valor óptimo de lambda es aproximadamente -3, que es el que minimiza el error de predicción. Este valor lambda dará el modelo más preciso. El valor exacto de lambda se puede ver como sigue:
...
SRL_pred_train <- predict(std_ridge_logit, x_train, type="class", s=cv_train$lambda.min)
...
```{r}
SRL_pred_test <- predict(std_ridge_logit, x_test, type="class", s=cv_train$lambda.min)
