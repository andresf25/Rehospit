...
x <- model.matrix(rehosp_oms~., training)[,-1]
y <- ifelse(training$rehosp_oms == "1", 1, 0)
...
La gráfica muestra el error de validación cruzada de acuerdo con el registro de lambda. La línea vertical punteada a la izquierda indica que el registro del valor óptimo de lambda es aproximadamente -9, que es el que minimiza el error de predicción. Este valor lambda dará el modelo más preciso. El valor exacto de lambda se puede ver como sigue:

```{r}
cv.lasso$lambda.min
```

En general, el propósito de la regularización es equilibrar la precisión y la simplicidad. Esto significa, un modelo con el menor número de predictores que también da una buena precisión. Para este fin, la función cv.glmnet() también encuentra el valor de lambda que proporciona el modelo más simple pero también se encuentra dentro de un error estándar del valor óptimo de lambda. Este valor se llama lambda.1se.
...
Con lambda.min ningún coeficiente es igual a cero.
...
Usando lambda.1se, solo el coeficiente de una variable se ha establecido en cero mediante el algoritmo de lazo, reduciendo la complejidad del modelo.
...
x.test <- model.matrix(rehosp_oms ~., testing)[,-1]
...
###Matriz training
```{r}
SRL_pred_train <- predict(model, x, type="class", s=cv.lasso$lambda.min)
confusion_matrix_train <- table(y, SRL_pred_train)
confusion_matrix_train
```

```{r}
error_rate_train <- (544+1453)/5720.0
error_rate_train
```
...
```{r}
SRL_pred_test <- predict(model, x_test, type="class", s=cv.lasso$lambda.min)
confusion_matrix_test <- table(y_test, SRL_pred_test)
confusion_matrix_test
```

```{r}
error_rate_test <- (1605+143)/10424.0
error_rate_test
```
...
```{r}
# Model accuracy
observed.classes <- testing$rehosp_oms
mean(predicted.classes == observed.classes)
```

```{r}
#report mean error rate (fraction of incorrect labels)
confusion_matrix_train <- table(as.matrix(testing[, 7]), y_pred > 0.5)

confusion_matrix_train <- table(testing, predicted.classes)
confusion_matrix_train
```


Calculando el modelo con lambda.1se:
...
#ModeloArbol <- rpart(rehosp_oms ~ ., data = training, method = 'class')
