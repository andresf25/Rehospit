...
La curva ROC no depende de la distribución de clase, lo que lo hace útil para evaluar clasificadores que predicen eventos raros como enfermedades o desastres. Por el contrario, la evaluación del rendimiento utilizando la precisión (TP + TN) / (TP + TN + FN + FP) favorecería a los clasificadores que siempre predicen un resultado negativo para la clase minoritaria.
...
```{r}
test_predict <- predict(orig_fit, newdata = testTransformed) 
confusionMatrix(reference = testTransformed$rehosp_oms, data = test_predictpredicted, positive = "si")
```
